...
namespace BTSaveEditor
{
    class Program
    {
        ...
        private static string tmpFolder = "BTSaveEditorTMP";
        ...
        public static bool ExtractFileFromSave(GZipStream compressionStream)
        {
            ...
            using (FileStream outFile = File.Open(fileName, System.IO.FileMode.Create, System.IO.FileAccess.Write))
            {
        ...
        {
            using (FileStream saveFileStream = File.Open(saveFile, System.IO.FileMode.Open, System.IO.FileAccess.Read))
                using (GZipStream compressionStream = new GZipStream(saveFileStream, CompressionMode.Decompress, true))
        ...
        {
            //args = new string[6] { "-w", "--session", "gamesession.xml", "--xml", "Lucia Kalor - WIP.sub.xml", "b.save"  };
